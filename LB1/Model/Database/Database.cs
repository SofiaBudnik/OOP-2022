...
namespace Lab1
{
    public class Database
    {
            ...
            using (GenericParser parser = new GenericParser())
            {
                ...
                parser.SetDataSource("Companies.txt");
                {
                    tableSet.Data.Tables["Companies"].Rows.Add(new object[] { parser["CompanyId"], parser["Type"],
                        parser["Name"], parser["UNP"], parser["BIC"], parser["Adress"] });
                ...
                while (parser.Read())
                {
                    tableSet.Data.Tables["Clients"].Rows.Add(new object[] { parser["UserID"], parser["Login"],
                        parser["Password"], parser["Name"], parser["Surname"],
                        parser["Patronymic"], parser["PhoneNumber"], parser["Email"],
                        parser["SNP"], parser["PassportID"], parser["Company"], parser["Approved"], parser["Cancelled"] });
        ...
        public void AddClient(string userId, string login, string password, string firstName, string surName, string patronymic, string phoneNumber, string email, string snp, string passportid, string company, bool apprv = true, bool cancl = false)
        {
        ...
        public void AddCompany(string id, string type, string name, string UNP, string BIN, string Adress)
        {
        ...
        public Client FindByLoginAndPassword(string login, string password)
        {
            ...
            if (row.Length != 0)
            {
                FClient.GetClient(
                   ...
                   Convert.ToString(row[0]["SNP"]),
                   Convert.ToString(row[0]["PassportID"]),
        ...
        {
            Administrator New = new Administrator();
